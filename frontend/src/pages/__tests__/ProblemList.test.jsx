...
describe("ProblemList", () => {
  it("renders problems returned by the API", async () => {
    ...
    expect(
      await screen.findByRole("link", { name: /perfect point-to-point link/i }),
    ).toHaveAttribute("href", "/problems/perfect-link");
    expect(screen.getByText("easy")).toBeInTheDocument();
    expect(screen.getByText("retransmission")).toBeInTheDocument();

    await waitFor(() =>
      expect(fetch).toHaveBeenCalledWith("/api/problems", undefined),
    );
  });
});
