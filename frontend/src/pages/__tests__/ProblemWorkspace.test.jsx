...
describe("ProblemWorkspace", () => {
  it("renders markdown and switches editable template tabs", async () => {
    ...
    fireEvent.change(helperEditor, {
      target: { value: "package solution\n\nfunc changed() {}\n" },
    });
    expect(screen.getByText("Unsaved changes")).toBeInTheDocument();

    await userEvent.click(screen.getByRole("tab", { name: "solution.go" }));
    expect(screen.getByLabelText("editor-solution.go")).toHaveValue(
      "package solution\n\nfunc Solve() {}\n",
    );

    await userEvent.click(screen.getByRole("tab", { name: "helper.go" }));
    expect(screen.getByLabelText("editor-helper.go")).toHaveValue(
      "package solution\n\nfunc changed() {}\n",
    );
  });
  ...
  it("loads a saved solution and saves edits", async () => {
    ...
    expect(fetch).toHaveBeenLastCalledWith(
      "/api/problems/perfect-link/solution",
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          files: { "solution.go": "package solution\n\nfunc Changed() {}\n" },
        }),
      },
    );
