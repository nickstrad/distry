...
func (n *node) HandleMessage(ctx sim.Context, from sim.NodeID, msg sim.Message) {
	...
	switch msg.Type {
	case AppBroadcast:
		n.accept(ctx, ctx.Self(), payload)
	case Data:
		n.accept(ctx, from, payload)
	}
}

func (n *node) HandleTimer(sim.Context, string) {}

func (n *node) accept(ctx sim.Context, from sim.NodeID, payload string) {
	n.ack(payload, from)
	if !n.pending[payload] {
		n.relay(ctx, payload)
	}
...
func (n *node) relay(ctx sim.Context, payload string) {
	n.pending[payload] = true
