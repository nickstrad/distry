...
CREATE TABLE submissions (
  ...
  compile_output text,
  reports      jsonb,                -- []simtest.Report; large traces are capped
...
1. Create temp workspace: `go.mod` (module `submission`, `replace distry =>` the server's
   repo root), user files under `solution/`, problem `harness/` files, and a fixed
   `main.go` shim. The shim passes `solution.New` into the harness, adapts matching
   dependency fields by name, and accepts `-full-trace` for replay.
2. `go build -o run ./` with `CGO_ENABLED=0`, `GOMEMLIMIT=256MiB`, `GOPROXY=off`,
   `GOSUMDB=off`, and `GOFLAGS=-mod=mod`, under a compile timeout. Compile errors →
   status `error` with cleaned output (strip workspace paths).
...
4. Overall status: all seeds passed ⇒ `passed`, any violation ⇒ `failed`. Stored traces
   are capped and reports set `truncated: true` when events were omitted.
...
- `Service` with injected deps: solutions repo, problems repo (for harness files + run
  ...
  so runs are async.
- `POST /api/problems/{slug}/run` with optional `{ "seeds": [...] }` → snapshot saved
  solution → insert `queued` → enqueue → return `{submissionID}`. Custom runs validate
  1-20 non-negative seeds. Reject if user has a submission already
  `queued|compiling|running` for this problem (409).
...
- `POST /api/submissions/{id}/replay` with `{seed}` — reruns one seed from the
  submission's stored file snapshot with full trace.
...
`failed` with a named violation and a trace for the failing seed; replaying the same seed
from that submission reproduces it.
