...
| Plan | Slice | Depends on |
|------|-------|-----------|
| 01 | Foundations: layout, config, DB pool, migrations, health check | — |
| 02 | Auth: Go email/password users + sessions + login UI | 01 |
| 03 | Problem domain: manifest format, loader, DB, list/detail API | 01 |
| 04 | Frontend problem browser: list + detail + read-only editor | 02, 03 |
| 05 | Solution persistence: save/load user files per problem | 02, 03 (UI part: 04) |
| 06 | Simulator core (`pkg/sim`): pure library, no deps on rest of app | — (repo layout from 01) |
| 07 | Invariant checkers & harness contract (`pkg/simtest`) | 06 |
| 08 | Runner & submission API: compile user code + harness, execute, store results | 03, 05, 06, 07 |
| 09 | Run/results UI: Run button, status polling, per-seed results, trace viewer | 04, 05, 08 |
| 10 | Problem 1 (easy): Perfect Link (retransmit + dedup) | 07 (test locally), 08+09 (E2E) |
| 11 | Problem 2 (easy): LCR Leader Election on a ring | same as 10 |
...
press Run, and see pass/fail per seed with invariant-violation details and an event trace
for any failing seed; re-running with the same seed reproduces the result exactly.
